...
Older, self-contained session write-ups moved out of `HANDOFF.md` on
2026-07-13 to keep that file readable in one pass (it had grown to 2192
lines). Nothing here is summarized or edited — every section below is
the verbatim content that used to live in `HANDOFF.md`, in its original
order. `HANDOFF.md` itself keeps the current entries list, an up-to-date
"Current objective," and the still-relevant "Do not touch"/"Known
